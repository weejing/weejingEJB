...
public class OssManagerBean implements OssManagerBeanRemote {
    ...
    private ArrayList listOfCartItems =null;
    ...
    public List<Vector> productOrderReport()
    {
        ...
        Query query = em.createQuery("SELECT o FROM Orders o JOIN "
                + "o.products p WHERE (p.brand = :brand) and (p.model = :model) "
                + "and (o.orderTime LIKE :month)")
                .setParameter("orderTime",month)
                .setParameter("brand", brand)
                .setParameter("model",model);
        
        System.out.println("Excuted query:" +query);
    ...
    */
    ...
    public boolean addToSearch(HashMap hash)
    {
         if(productEntity.getStockQuantity() ==0)
             return false;
         if(!(hash.get("type").equals("")) && !(productEntity.getProductType().equals(hash.get("type"))))
             return false;        
         if(!(hash.get("brand").equals("")) && !(productEntity.getBrand().equals(hash.get("brand"))))
             return false;             
         if(!(hash.get("model").equals(""))&& !(productEntity.getModel().equals(hash.get("model"))))
             return false;
         if(!(hash.get("minPrice").equals("")) && !(hash.get("maxPrice").equals("")) && 
                 productEntity.getUnitPrice() > Double.parseDouble((String)hash.get("max")) 
                 && productEntity.getUnitPrice() < Double.parseDouble((String)hash.get("min")))
             return false;
    ...
    */
    ...
    @Override
    public void payImmediate(String cardType, String cardName, int cardNum)
    {
    ...
    @Override
    public void latePayment(String cardType, String cardName, int cardNum, long orderId)
    {
